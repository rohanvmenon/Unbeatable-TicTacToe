...
class Board():
    ...
    def game_loop(self):
        ...
        print('  Move format: Enter a number between 0 and 8 (0 = top-left, 8 = bottom-right)')
        ...
        while True:
            ...
            try:
                # parse user input as a single number (0-8)
                move = int(user_input)
                if move < 0 or move > 8:
                    print(' Illegal move! Enter a number between 0 and 8.')
                    continue
                
                # calculate row and column from the input number
                row = move // 3
                col = move % 3
